...
**Phase 2A 完了（2026年1月19日）**:
- ✅ `RAGEngine.extract_perspective()`: キャラクター視点のブロック抽出
- ✅ `RAGEngine.search(character=)`: 検索時に自動で視点抽出を適用
- ✅ `Character.respond()`: RAG検索時に `character=self.name` を渡す

**知識ファイルの状況**:
- `knowledge/jetracer_tech_with_perspectives.txt`: ✅ 視点ブロック形式
- `knowledge/jetracer_tech.txt`: ❌ 視点なし（Phase 2B対象）
- `knowledge/sisters_shared.txt`: ❌ 視点なし（Phase 2B対象）

**残りの作業（Phase 2B）**:
- 全知識ファイルの視点形式への移行（2ファイル対象）
...
| F-004 | 視点つきRAGフィルタ | 中 | ✅ 完了 | Phase 2A実装済み（TC-R-009〜014, TC-C-010〜011で検証） |
| F-005 | 全知識ファイルの視点形式移行 | 中 | ❌ 未実装 | 2ファイル対象（Phase 2B） |
